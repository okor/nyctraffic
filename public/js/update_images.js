$(function(){
  ...
  var selected_cam_size = cam_sizes['medium']
  ...
  function appendRender(region){
		...
		$.each(region, function(index, cam){
			cam_details = {
			  name: cam['name'],
			  short_name: short_title(cam['name'],max_title_length),
			  url: cam['url'],
			  time: time
			}
		  if (cam_details['url'] !== ""){
	...
	function short_title(s,max_length){ 
  ...
  function update_images(dimensions){
		var images = $("#cams img")
		var time = Date.now().toString()
		$.each(images, function(index, img){
		  var src = $(img).attr('src')
		  $(img).attr('src', src.split('?')[0]+'?'+time ).attr('width', dimensions[0]).attr('height', dimensions[1])
		})
  } 

  function start(region,dimensions){
  	appendRender(region)
  	update_images(dimensions)
  ...
  function toggle_pause(button){
		if (pause == false){
		  ...
		  pause = false
			start(region, selected_cam_size)
		}
  }
  ...
  $("#size_chooser").bind('change', function(){
		selected_cam_size = cam_sizes[$(this).val()]
		stop()
		start(region, selected_cam_size)
  })
